...
<body class="standard">
    <div id="header">
        ...
        <h1 id="title">Just do it.<div id="border"></div>
        </h1>
    </div>

    <div id="form">
        <form class="input_form" method="post" action="">
            <input class="todo-input" type="text" placeholder="Add a task." name="task">
            <button class="todo-btn" type="submit" name="create">I Got This!</button>
        </form>
    </div>
        ...
        <div id="myUnOrdList">
            <ul class="todo-list">
                ...
                foreach ($tasks as $index => $task) {
                    ...
                    $task_name = $info[0];
                    $status = $info[1];
                    $completed_style = $status == "done" ? "completed" : "";
                    echo "<div class='todo standard-todo'>";
                    echo "<li class='todo-item $completed_style'>$task_name</li>";
                    echo "<form method='post' action=''><button class='delete-btn standard-button' name='delete' value='$index'><i class='fas fa-trash'></i></button></form>";
                    echo "<form method='post' action=''><button class='check-btn standard-button' name='check' value='$index'><i class='fas fa-check'></i></button></form>";
                    echo "</div>";
                }
                ?>
            </ul>
